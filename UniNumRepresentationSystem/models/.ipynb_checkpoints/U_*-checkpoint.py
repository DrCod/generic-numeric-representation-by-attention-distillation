...
from .seq2seq_T_* import T*Model
...
class UniversalRepresentationModel(nn.Module):
    ...
    super(UniversalRepresentationModel, ).__init__()
    ...
    if hparams.token_embedding == "hashing":
        self.token_embedder = MultiHashingEmbedder((hparams.hidden_size, slice_count = hparams.embedder_slice_count, 
                                                                       bucket_count =  hparams.embedder_bucket_count))
    elif hparams.token_embedder == "normal":
        self.token_embedder = nn.Embedding(vocab_size, hparams.hidden_size)
    else:
        RuntimeError(f"Invalid token embedder selected . Token embedder must be in [hashing, normal]")
    ...
    def forward(self, x):
        ...
        return {'embeddings': final_embeddings,
